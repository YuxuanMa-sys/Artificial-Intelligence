...
class q_learner():
    ...
    def choose_unexplored_action(self, state):
        ...
        '''
        a = self.report_exploration_counts(state)
        actions = []
        for i in range(-1, 2):
            if(a[i + 1] < self.nfirst):
                actions.append(i)
        if len(actions) <= 0:
            return None
        else:
            choice = random.choice(actions)
            self.N[state[0], state[1], state[2], state[3], state[4], choice+1] += 1
            return choice

        #return self.report_exploration_counts(state)
    ...
    def q_local(self, reward, newstate):
        ...
        '''
        q_local = reward + self.gamma * np.max(self.Q[newstate[0], newstate[1], newstate[2], newstate[3], newstate[4]])
        return q_local
    def learn(self, state, action, reward, newstate):
        ...
        None
        '''
        q = self.Q[state[0], state[1], state[2], state[3], state[4], action+1]
        q_local = self.q_local(reward, newstate)
        self.Q[state[0], state[1], state[2], state[3], state[4], action + 1] += self.alpha * (q_local - q)
    ...
    def exploit(self, state):
        ...
        '''
        Q = np.max(self.Q[state[0], state[1], state[2], state[3], state[4]])
        action = np.argmax(self.Q[state[0], state[1], state[2], state[3], state[4]]) - 1
        return action, Q
    ...
    def act(self, state):
        ...
        '''
        actions = self.choose_unexplored_action(state)
