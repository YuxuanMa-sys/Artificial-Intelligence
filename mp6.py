    ...
    if depth == 0 or len(moves) == 0:
        ...
        if side:
            ...
            for i in range(len(moves)):
                new_side, new_board, new_flags = makeMove(side, board, moves[i][0], moves[i][1], flags, moves[i][2])
                new_value, new_moveList, new_moveTree = minimax(new_side, new_board, new_flags, depth - 1)
                moveTree[encode(*moves[i])] = new_moveTree
                if value > new_value:
                    value = new_value
                    moveList = [moves[i]] + new_moveList
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if depth == 0 or len(moves) == 0:
        ...
        if side:
            ...
            for i in range(len(moves)):
                new_side, new_board, new_flags = makeMove(side, board, moves[i][0], moves[i][1], flags, moves[i][2])
                new_value, new_moveList, new_moveTree = alphabeta(new_side, new_board, new_flags, depth - 1, alpha, beta)
                moveTree[encode(*moves[i])] = new_moveTree
                if value > new_value:
                    value = new_value
                    moveList = [moves[i]] + new_moveList
